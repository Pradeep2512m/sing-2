...
fetchData(null, function (data) {
        ...
        const {
            images: { LARGE, REGULAR, SMALL, THUMBNAIL }, label: title, source: author, ingredients = [], totalTime: cookingTime = 0, calories = 0, cuisineType = [], dietlabels = [], dishtype = [], yield: servings = 0, ingredientLines = [], uri
        ...
        ingredientLines.map(ingredient => {
            ingredientItems += ` 
        <li class="ingr-items">${ingredient}</li>
                `;
        });
       ...
       <figure class="detail-banner img-holder">
            <img src="${bannerUrl}" alt="${title}" class="img-cover" width="${width}" height="${height}">
        </figure>
        ...
        <div class="detail-content">
            ...
            <div class="detail-author label-large">
                <span class="span">by<span>${author}</span>
            </div>
           ...
           Ingredients  <span class="label-medium">
           for ${servings} Servings</span>
           </h2>
